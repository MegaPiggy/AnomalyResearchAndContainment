...
namespace AnomalyResearchAndContainment
{
    public class ElevatorPad : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            {
                AnomalyResearchAndContainment.Instance.ModHelper.Console.WriteLine(nameof(ElevatorPad) + ".Update() in " + gameObject.name + " encountered Infinity value", OWML.Common.MessageType.Warning);
                return;
            }
            if (float.IsNaN(position.x) || float.IsNaN(position.y) || float.IsNaN(position.z))
            {
                AnomalyResearchAndContainment.Instance.ModHelper.Console.WriteLine(nameof(ElevatorPad) + ".Update() in " + gameObject.name + " encountered NaN value", OWML.Common.MessageType.Warning);
                return;
            }
